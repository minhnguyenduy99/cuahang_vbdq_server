...
import PhieuProps from "./PhieuProps";
import ChiTietPhieu from "../ctphieu/ChiTietPhieu";
...
export default abstract class Phieu extends AggrerateRoot<PhieuProps> {
  ...
  protected _listCTPhieu: ChiTietPhieu<any>[];
  protected _nhanVien: NhanVien;

  protected constructor(props: PhieuProps, nhanvien: NhanVien, listCTPhieu: ChiTietPhieu<any>[]) {
    super(props);
    if (!props.id) {
      props.id = uniqid();
    }
  ...
  protected getTongGiaTri(): number {
    ...
    return this._listCTPhieu.map(ctphieu => ctphieu.getGiaTri()).reduce((pre, cur) => pre + cur);
  }
  ...
  protected static async createPhieuProps<Prop extends PhieuProps>(type: new () => Prop, data: any, _listCTPhieu: ChiTietPhieu<any>[], nhanvien: NhanVien, createType: string) {
